...
public class Enemy : MonoBehaviour
{
    ...
    void Update()
    {
        checkStun(); // Обрабатываем стан
        move(); // Обрабатываем передвижение
        checkFlip(); // Обрабатываем повороты
    }

    protected void checkStun() // Проверка и исполнение стана
    {
        if (timer_stun <= 0) current_speed = normal_speed; // Если стана нет, возвращаем нормальную скорость
        else // Если есть стан
        {
            current_speed = 0; // Враг стоит на месте
            timer_stun -= Time.deltaTime; // Отсчитывается время до конца стана
        }
    }

    public void takeDamage (float damage) // Получение урона
    {
        timer_stun = stun; // Станим
    ...
    {
        if (player.transform.position.x > transform.position.x && !facing_right) // Если игрок справа, а враг смотрит влево
        {
            flip(); // Поворачиваем врага
            facing_right = true;

        }
        else if (player.transform.position.x < transform.position.x && facing_right) // Если игрок слева, а враг смотрит вправо
        {
            flip(); // Поворачиваем врага
            facing_right = false;
        }
